...
- [x] 12 módulos clínicos
- [x] Prontuário unificado
- [x] Analytics
...
| **Token** | Unidade de custo das APIs de IA |
...
Um LLM (Large Language Model) como Claude ou GPT **não pensa**. Ele **prediz a próxima palavra** baseada em probabilidades.

### Atenção (Attention)
O segredo deles é o mecanismo de atenção, que permite focar no que é relevante (ex: "tosse" + "seca" em "Golden Retriever").

## 2. Fluxo de uma Mensagem no VetCopilot

1. Usuário digita
2. Frontend capta e envia para API
3. API carrega contexto do Pet e System Prompt
4. Envia para o modelo (Claude)
5. Modelo decide se precisa de Tools (dados do banco)
6. Executa Tool e devolve resultado para a IA
7. IA gera resposta final via Streaming

## 3. Tokens e Custos

IA paga por token (pedação de palavra).
- 1 token ≈ 0.75 palavras em inglês
- AgendaVet otimiza enviando apenas dados clínicos essenciais no contexto.

## 4. System Prompt: A Programação da IA

É onde definimos a personalidade: "Você é o VetCopilot...". No AgendaVet, o prompt garante que a IA nunca dê diagnósticos definitivos e sempre cite que a decisão final é do veterinário.

## 5. Tool Calling: Acesso a Dados Reais

A IA tem "ferramentas" para:
- \`get_pet_info\`: dados básicos
- \`get_medical_history\`: histórico clínico
- \`calculate_dosage\`: farmacologia segura

## 6. RAG (Retrieval Augmented Generation)

Fase 2 do projeto: busca semântica em documentos (WSAVA, AAHA). Permite que a IA responda baseada em diretrizes que não estavam no seu treinamento original.

## 7. Arquitetura do VetCopilot
...
### Mapa de Evolução
1. MVP (Atual): Chat + Prontuário
2. Fase 2: RAG + Diretrizes
3. Fase 3: Interpretação de Exames
4. Fase 4: Visão Computacional (Radiografias)
