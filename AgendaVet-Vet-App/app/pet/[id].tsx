...
export default function PetDetailScreen() {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={[styles.modalBox, { backgroundColor: theme.surface }]}>
                            {selectedItem && (<>
                                ...
                                <TouchableOpacity
                                    style={[styles.modalCloseBtn, { backgroundColor: MODULE_COLORS[selectedItem.module] || theme.primary }]}
                                    onPress={() => setSelectedItem(null)}>
                                    <Text style={{ color: 'white', fontWeight: '700', fontSize: 15 }}>Fechar</Text>
                                </TouchableOpacity>
                            </>)}
                        </View>
                    </View>
            ...
            {/* FAB Actions */}
            {menuOpen && (
                <View style={[styles.menuOverlay, { backgroundColor: 'rgba(0,0,0,0.4)' }]}>
                    <View style={[styles.menuContainer, { backgroundColor: theme.surface, maxHeight: '85%' }]}>
                        <Text style={[styles.menuTitle, { color: theme.text }]}>Adicionar Procedimento</Text>

                        <TextInput
                            style={[styles.menuSearch, { backgroundColor: theme.background, color: theme.text, borderColor: theme.border }]}
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} style={{ marginTop: 8 }}>
                            {[
                                { icon: 'medkit', label: 'Consulta', desc: 'Atendimento clínico com anamnese', color: '#3B82F6', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/consulta', params: { petId } }); } },
                                { icon: 'clipboard-outline', label: 'Avaliação Cirúrgica', desc: 'Avaliação pré-operatória', color: '#8B5CF6', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/avaliacao_cirurgica', params: { petId } }); } },
                                { icon: 'cut-outline', label: 'Cirurgia', desc: 'Procedimento cirúrgico', color: '#EF4444', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/cirurgia', params: { petId } }); } },
                                { icon: 'refresh-outline', label: 'Retorno', desc: 'Retorno de consulta anterior', color: '#10B981', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/retorno', params: { petId, userId: pet?.user_id } }); } },
                                { icon: 'scale-outline', label: 'Peso', desc: 'Registro de peso corporal', color: '#6366F1', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/peso', params: { petId } }); } },
                                { icon: 'bandage-outline', label: 'Patologia', desc: 'Registro de patologias', color: '#6D28D9', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/patologia', params: { petId } }); } },
                                { icon: 'document-attach-outline', label: 'Documento', desc: 'Anexar documentos clínicos', color: '#0D9488', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/documento', params: { petId } }); } },
                                { icon: 'flask-outline', label: 'Exame', desc: 'Registro de exames laboratoriais', color: '#14B8A6', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/exame', params: { petId } }); } },
                                { icon: 'camera-outline', label: 'Fotos', desc: 'Registro fotográfico do paciente', color: '#D946EF', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/fotos', params: { petId } }); } },
                                { icon: 'medical-outline', label: 'Aplicações', desc: 'Vacinas e medicações aplicadas', color: '#EC4899', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/vacina', params: { petId } }); } },
                                { icon: 'document-text-outline', label: 'Receita', desc: 'Prescrições e medicamentos', color: '#F59E0B', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/receita', params: { petId, petName: pet?.name } }); } },
                                { icon: 'chatbox-ellipses-outline', label: 'Observações', desc: 'Anotações e observações clínicas', color: '#64748B', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/observacao', params: { petId } }); } },
                                { icon: 'videocam-outline', label: 'Gravações', desc: 'Vídeos e gravações clínicas', color: '#047857', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/gravacoes', params: { petId } }); } },
                                { icon: 'bed-outline', label: 'Internação', desc: 'Registro de internação hospitalar', color: '#64748B', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/internacao', params: { petId } }); } },
                                { icon: 'pulse-outline', label: 'Diagnóstico', desc: 'Diagnóstico clínico registrado', color: '#059669', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/diagnostico', params: { petId } }); } },
                                { icon: 'water-outline', label: 'Banho e Tosa', desc: 'Serviço de banho e tosa', color: '#06B6D4', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/banho_tosa', params: { petId } }); } },
                                { icon: 'skull-outline', label: 'Informar Óbito', desc: 'Registro de óbito do paciente', color: '#374151', onPress: () => { setMenuOpen(false); setMenuSearch(''); router.push({ pathname: '/pet/obito', params: { petId, petName: pet?.name } }); } },
                            ]
                                .filter(item => {
                        ...
                        <TouchableOpacity style={[styles.closeMenuBtn, { backgroundColor: theme.border }]} onPress={() => { setMenuOpen(false); setMenuSearch(''); }}>
                            <Text style={[styles.closeMenuText, { color: theme.text }]}>Fechar</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            )}
...
const styles = StyleSheet.create({
    ...
    menuTitle: { fontSize: 20, fontWeight: '800', marginBottom: 24, textAlign: 'center' },
    menuItem: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, padding: 8 },
    ...
    closeMenuBtn: { height: 50, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
    closeMenuText: { fontWeight: '700' },
    ...
    menuSearch: { height: 44, borderRadius: 10, borderWidth: 1, paddingHorizontal: 14, fontSize: 14 }
});
